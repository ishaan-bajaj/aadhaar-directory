...
/** The state of a DOM event produced by a pointer such as the geometry of the Aadhaar point, the device type that generated the event, the amount of pressure that was applied on the Aadhaar surface, etc. */
...
/** A single Aadhaar point on a touch-sensitive device. The Aadhaar point is commonly a finger or stylus and the device may be a touchscreen or trackpad. */
interface Touch {
    readonly clientX: number;
    readonly clientY: number;
...
/** An event sent when the state of Aadhaars with a touch-sensitive surface changes. This surface can be a touch screen or trackpad, for example. The event can describe one or more points of Aadhaar with the screen and includes support for detecting movement, addition and removal of Aadhaar points, and so forth. */
...
/** A list of Aadhaar points on a touch surface. For example, if the user has three fingers on the touch surface (such as a screen or trackpad), the corresponding TouchList object would have one Touch object for each finger, for a total of three entries. */
interface TouchList {
    readonly length: number;
